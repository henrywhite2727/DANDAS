...
def DANDAS(DM_mass, Halo_Profile, Antiparticle_Nature, Decay_Process,plot,data,**kwargs):
    ...
    if Halo_Profile=='NFW' or Halo_Profile=='NFW Custom Quick':
        ...
        gamma = 1.2             # slope parameter
            
        #If NFW profile is not customized, the J and D factors do not have to be "re-scaled"
        if Halo_Profile=='NFW':
            scaling_factor=1
            scaling_factor_J=1
        
        #If NFW parameters are customized, a new scale density is needed to "re-scale" the J and D factors 
        elif Halo_Profile=='NFW Custom Quick':
            #Importing custom parameters from user
            r0_custom = kwargs.get('r0', None) * 3.0857e21
            rho_0_custom = kwargs.get('rho_0', None)
        
            #Calculating scale density based on custom parameters
            rho_s_custom = rho_0_custom / ((2**(3 - gamma))/(((r0_custom/rs)**gamma)*(1 + (r0_custom/rs))**(3 - gamma))) # [GeV/cm^-3]
            
            #Calculating scale density for best fit parameters
            rho_s = rho_0 / ((2**(3 - gamma))/(((r0/rs)**gamma)*(1 + (r0/rs))**(3 - gamma))) # [GeV/cm^-3]
            
            #calculating ratio of scale densities to get scaling factor
            
            scaling_factor=rho_s_custom/rho_s
            
            scaling_factor_J=(rho_s_custom**2)/(rho_s**2)  #Different scaling for J factors as density is squared in that integral
        ...
        if Halo_Profile=='Einasto_custom':
            ...
            #Calculating scale density for best fit values for r0 and rho_0 parameters
            rho_s_ein = rho_0 / np.exp((-2/alpha)*(((r0/rs)**alpha)-1))
            
            #Calculating scale density for custom parameters
            rho_s_ein_custom = rho_0_custom / np.exp((-2/alpha)*(((r0_custom/rs)**alpha)-1))
            
            scaling_factor=rho_s_ein_custom/rho_s_ein
            scaling_factor_J=(rho_s_ein_custom**2)/(rho_s_ein**2)  #Different scaling for J factors as density is squared in that integral
        
        D_allsky=2.7236e+23*scaling_factor
        J_allsky=2.22e23*scaling_factor_J
        
        D_anita=5.431e+21*scaling_factor
        J_anita=2.0084e21*scaling_factor_J
        
        D_grand=3.011e+22*scaling_factor
        J_grand=2.591e+22*scaling_factor_J
        
        D_cta=3.145e20*scaling_factor
        J_cta=9.732e+21*scaling_factor_J
        
        D_tambo=1.0284e+20*scaling_factor
        J_tambo=8.63e+19*scaling_factor_J
        
        J_auger1=9.51e+21*scaling_factor_J            #for [90,95] degree range
        J_auger2=2.665e+22*scaling_factor_J           #for [75,90] degree range
        J_auger3=2.462e+22*scaling_factor_J           #for [60,75] degree range
        D_auger1=1.1518e+22*scaling_factor          #for [90,95] degree range
        D_auger2=3.504e+22*scaling_factor           #for [75,90] degree range
        D_auger3=3.435e+22*scaling_factor           #for [60,75] degree range   
        
        J_pone1=7.56e+22*scaling_factor_J    #for cos(zenith)=[-1,-0.5]
        J_pone2=1.169e+23*scaling_factor_J    #for cos(zenith)=[-0.5,0.5]
        J_pone3=1.2659e+22*scaling_factor_J    #for cos(zenith)=[0.5,1]
        D_pone1=8.328e+22*scaling_factor    #for cos(zenith)=[-1,-0.5]
        D_pone2=1.383e+23*scaling_factor    #for cos(zenith)=[-0.5,0.5]
        D_pone3=4.818e+22*scaling_factor    #for cos(zenith)=[0.5,1]
        
        
            
    elif Halo_Profile=='NFW Custom Long' or Halo_Profile=='Einasto Custom Long' or 'Custom Density Function':        
        ...
        # ======================================================================
                ...
                def rho_DM(ra,dec,x):
                    ...
                    if Halo_Profile=='NFW Custom Long':
                    ...
                    elif Halo_Profile=='Einasto Custom Long':
        ...
        J_allsky=allsky_dfactor.jfactor
        ...
        D_grand=grand_dfactor.dfactor
        J_grand=grand_dfactor.jfactor
        
        anita_dfactor = Dfactor('anita')
        D_anita=anita_dfactor.dfactor
        J_anita=anita_dfactor.jfactor
        

        
        tambo_dfactor = Dfactor('tambo')
        D_tambo=tambo_dfactor.dfactor
        J_tambo=tambo_dfactor.jfactor
        
        #Adding arbitrary counting variable i as loop is over exp
        i=0
        D_auger=np.zeros(3)
        J_auger=np.zeros(3)
        for exp in range(3):
            auger_dfactor = Dfactor('auger')
            D_auger[i]=auger_dfactor.dfactor
            J_auger[i]=auger_dfactor.jfactor
            i=i+1
        ...
        J_auger3=J_auger[2]
        '''
        #Adding arbitrary counting variable k as loop is over exp
        '''
    ...
    #Calculating limit directly from https://pos.sissa.it/301/972/pdf
    auger_Enu=Auger_Paper[:,0]*1e-9                      #converting from eV to GeV
    auger_fluxd=(Auger_Paper[:,1]/(auger_Enu**2))            #converting to diffuse flux
    auger_DM_mass=auger_Enu*2


    Auger_taud=np.zeros(len(D_auger))
    ...
    tau_cta=rescaling(cta_mass,cta_sigmaV,J_cta,D_cta)
    ...
    def get_signal_expectation(D_allsky, tau, E_nu):
        ...
        #constant = 1./(4.*np.pi) * sigmav / (kappa * mx**2) * 1/3. * J_allSky / 2.

        #altering above expression to give decay limits instead of annihilation ones

        #factor of (1/2) is due to sensitivity to nue_bar
    ...
    def log_poisson(k, mu):
        ...
        #My code with added if statement to avoid log(0) error
        
        #POTENTIALLY UNCOMMENT THIS OUT LATER
...
#checking annihilation limits don't have weird factor of 2 issue before I import into DANDAS
bor_db=np.loadtxt('BorexinoAnnihilationLimits.txt')
Bor_Kam_Sknu_sigmaV_doublecheck=np.loadtxt('Bor_Kam_Sknu_sigmaV_doublecheck.csv',delimiter=',')
kamData2=np.loadtxt('kamland.txt')

#Importing the SK-nuebar annihilation cross section data
Sk_nuebar_Data=np.loadtxt('SK4_nuebar.csv', delimiter=',')
Sk_nuebar_mass=(Sk_nuebar_Data[:,0])   #factor of 2 is for decay
Sk_nuebar_sigmaV=Sk_nuebar_Data[:,1] 



plt.figure(2)
plt.plot(bor_db[:,0]/1000,bor_db[:,1]/2,label='txt file')
plt.plot(Bor_Kam_Sknu_sigmaV_doublecheck[:,0],Bor_Kam_Sknu_sigmaV_doublecheck[:,1],label='digitized double check')
plt.plot(kamData2[:,0]/1000,kamData2[:,1],label='kam')
plt.plot(Sk_nuebar_mass,Sk_nuebar_sigmaV,label='sk4')
plt.xscale("log")
plt.yscale("log")
plt.legend()
...
# This section aims to calculate J and D factors for a range of experiments given different DM halo profiles

# -------------------- Author: Diyaselis Delgado López -------------------------
# -------------------- Adapted by: Henry White ---------------------------------
# ==============================================================================
...
class Dfactor:
    ...
    def __init__(self,experiment):
        ...
        def rho_DM(ra,dec,x):
            ...
            # calculating scale densitity rho_s [GeV/cm^-3] for NFW profile
            rho_s = rho_0 / ((2**(3 - gamma))/(((r0/rs)**gamma)*(1 + (r0/rs))**(3 - gamma))) # [GeV/cm^-3]
            
            # calculating DM densitity rho_DM [GeV/cm^-3] based on NFW profile
            rho_DM = rho_s * (2**(3 - gamma))/(((r/rs)**gamma)*(1 + (r/rs))**(3 - gamma)) # [GeV/cm^-3]
            
            #calculating scale density for Einasto profile
            
            alpha=0.155 #refer to Thesis Notebook for justification
            rho_s_ein = rho_0 / np.exp((-2/alpha)*(((r0/rs)**alpha)-1))
            
            #Calculating DM density for Einasto profile
            rho_DM_ein=rho_s_ein * np.exp((-2/alpha)*(((r/rs)**alpha)-1))
...
'''
allsky_dfactor = Dfactor('allsky')
print('Decay integration summary:\n', allsky_dfactor.dfactor.summary())
print('The', allsky_dfactor.experiment ,'D factor (decay) is {} GeV cm^-2 sr with an error of {:.2f}%. \n'.format(allsky_dfactor.dfactor,allsky_dfactor.error * 100))

anita_dfactor = Dfactor('anita')
print('Decay integration summary:\n', anita_dfactor.dfactor.summary())
print('The', anita_dfactor.experiment ,'D factor (decay) is {} GeV cm^-2 sr with an error of {:.2f}%. \n'.format(anita_dfactor.dfactor,anita_dfactor.error * 100))

grand_dfactor = Dfactor('grand')
print('Decay integration summary:\n', grand_dfactor.dfactor.summary())
print('The', grand_dfactor.experiment ,'D factor (decay) is {} GeV cm^-2 sr with an error of {:.2f}%. \n'.format(grand_dfactor.dfactor,grand_dfactor.error * 100))

cta_dfactor = Dfactor('cta')
print('Decay integration summary:\n', cta_dfactor.dfactor.summary())
print('The', cta_dfactor.experiment ,'D factor (decay) is {} GeV cm^-2 sr with an error of {:.2f}%. \n'.format(cta_dfactor.dfactor,cta_dfactor.error * 100))

tambo_dfactor = Dfactor('tambo')
print('Decay integration summary:\n', tambo_dfactor.dfactor.summary())
print('The', tambo_dfactor.experiment ,'D factor (decay) is {} GeV cm^-2 sr with an error of {:.2f}%. \n'.format(tambo_dfactor.dfactor,tambo_dfactor.error * 100))
'''
# Auger exposure selection
exp_auger = ['[95º,90º]','[90º,75º]','[75º,60º]']
for exp in range(3):
    auger_dfactor = Dfactor('auger')
    print('Decay integration summary:\n', auger_dfactor.dfactor.summary())
    print('The', auger_dfactor.experiment ,'D factor (decay) for',exp_auger[exp],'is {} GeV cm^-2 sr with an error of {:.2f}%. \n'.format(auger_dfactor.dfactor,auger_dfactor.error * 100))

# P-ONE exposure selection
exp_pone = ['[-1,-0.5]','[-0.5,0.5]','[0.5,1]']
for exp in range(3):
    pone_dfactor = Dfactor('pone')
    print('Decay integration summary:\n', pone_dfactor.dfactor.summary())
    print('The', pone_dfactor.experiment ,'D factor (decay) for ', exp_pone[exp],'is {} GeV cm^-2 sr with an error of {:.2f}%. \n'.format(pone_dfactor.dfactor,pone_dfactor.error * 100))
